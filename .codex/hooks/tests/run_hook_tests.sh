#!/bin/bash
# Self-tests for .codex/hooks/*.sh
# 実行: bash .codex/hooks/tests/run_hook_tests.sh
...
# Codex apply_patch 相当の payload 生成
payload_for() {
  $PY -c "
import json, sys
...
# apply_patch ペイロード生成ヘルパー（python で json.dumps）
build_edit_payload() {
  local file_path="$1"
  local new_string="$2"
  $PY -c "
...
print(json.dumps({
  'tool_input': {
    'command': '*** Begin Patch\\n*** Update File: ' + sys.argv[1] + '\\n@@\\n' + ''.join('+' + line + '\\n' for line in sys.argv[2].splitlines()) + '*** End Patch'
  }
}))
...
echo "${YELLOW}== .codex/hooks セルフテスト ==${RESET}"
echo "repo: ${ROOT}"
echo "python: ${PY}"
echo
...
case4_ok=true
for exempt_path in "${ROOT}/docs/hooktest.md" "${ROOT}/.codex/hooktest.sh"; do
...
output6=$(echo "$payload6" | HOOK_PROJECT_ROOT="$SANDBOX" bash "$HOOKS/remind_docs_update.sh" 2>&1)
exit6=$?
...
# ─────────────────────────────────────────
# Case 8: hooks.json と helper の構文・参照整合性
# ─────────────────────────────────────────
echo "--- Case 8: hooks.json と helper の構文・参照整合性 ---"

if $PY -m py_compile "$HOOKS/hook_input.py" 2>/dev/null && \
   $PY -c "
...
for groups in config['hooks'].values():
    for group in groups:
        for hook in group['hooks']:
            commands.extend([hook['command'], hook.get('commandWindows', '')])
registered = {
    match.replace('\\\\', '/').rsplit('/', 1)[-1]
    for command in commands
    for match in re.findall(r'\.codex[\\\\/]+hooks[\\\\/]+[A-Za-z0-9_.-]+\.sh', command)
}
expected = {path.name for path in (root / '.codex' / 'hooks').glob('*.sh')}
assert registered == expected, (registered, expected)
" 2>/dev/null; then
  pass "Case 8: hooks.json・参照先・hook_input.py の構文 OK"
else
  fail "Case 8: hooks.json・参照先・hook_input.py の不整合"
fi
