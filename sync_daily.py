...
def check_docs(project_dir: str) -> str:
    prompt = (
        ...
        "2. docs/design/基本設計/基本設計書.md を Read\n"
        "3. 変更されたバックエンド・フロントエンドのファイルが設計書のファイルマップに反映されているか確認\n"
        "4. backend/routers/ 配下の主要エンドポイントが docs/design/詳細設計/API.md（一覧は /openapi.json）に記載されているか確認\n\n"
        "## 出力形式（このフォーマットで出力してください）\n"
        "## 設計書整合性チェック結果\n"
        "### ✅ 整合済み\n"
        "- (整合している項目)\n"
