...
# 事実抽出は出力余地を残すため、1回の入力をone-shot要約より小さくする。
# 分割はページ境界で行い、上限を超えた本文も全ブロック処理する。
FACT_CHUNK_MAX_CHARS = 120_000
...
完成したあらすじや人物紹介を書く前の材料として、本文で確認できる事実を抽出してください。
...
- 完成した要約文や人物紹介文にはせず、後段で文章化できる具体的な事実を残す。
