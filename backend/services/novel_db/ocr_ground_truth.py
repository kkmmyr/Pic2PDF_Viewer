    ...
    with with_db() as conn:
        with conn:
            for sample in samples:
                row = conn.execute(
                    "SELECT r.book_name, p.image_sha256, p.page_type "
                ...
                cursor = conn.execute(
                    ...
                    INSERT INTO ocr_ground_truth_pages (
                        run_id, page_no, image_sha256, page_type, state, created_at, updated_at
                    ) VALUES (?, ?, ?, ?, 'draft',
    ...
    with with_db() as conn:
        ...
        if state == "verified":
            cursor = conn.execute(
                ...
                SET reference_text=?, page_type=?, state=?, note=?,
                    updated_at=datetime('now', '+9 hours'),
                    verified_at=datetime('now', '+9 hours')
                WHERE id=?
                """,
                (reference_text, page_type, state, note, entry_id),
            )
        else:
            cursor = conn.execute(
                """
...
def list_ground_truth() -> dict:
    with with_db() as conn:
        rows = conn.execute(
            ...
            SELECT g.id, g.run_id, g.page_no, g.image_sha256, g.page_type,
                   g.reference_text, g.state, g.note, g.created_at, g.updated_at,
                   g.verified_at, r.book_name, p.full_text
    ...
    for row in rows:
        reference_text = str(row[5] or "")
        ocr_text = str(row[12] or "")
        page_type = str(row[4])
