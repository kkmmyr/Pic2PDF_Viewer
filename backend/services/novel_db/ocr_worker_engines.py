...
try:
    ...
    from .ocr_provenance import candidate_manifest, collect_runtime_manifest
...
def process_surya_task(
    ...
    *,
    ...
    primary_ocr_ms = round((time.perf_counter() - primary_started) * 1000)
    ...
    result = surya_result
    external_result: SuryaPageResult | None = None
    ...
    if trigger_flags or crosscheck_all_pages:
        ...
        external_ocr_ms = round((time.perf_counter() - external_started) * 1000)
    ...
    candidates = candidate_manifest(
        ...
        primary_raw_output=surya_result.raw_output,
        primary_state=surya_result.state,
        primary_block_count=len(surya_result.blocks),
        primary_quality_flags=surya_result.quality_flags,
        primary_attempt_count=surya_result.attempt_count,
        external_text=external_result.full_text if external_result is not None else None,
        external_raw_output=external_result.raw_output if external_result is not None else None,
        external_state=external_result.state if external_result is not None else None,
        external_block_count=len(external_result.blocks) if external_result is not None else None,
        external_quality_flags=external_result.quality_flags if external_result is not None else None,
        external_attempt_count=external_result.attempt_count if external_result is not None else None,
    )
    ...
    return SuryaTaskExecution(
        ...
        processing_timing={
            ...
            "external_ocr_ms": external_ocr_ms,
...
def run_yomitoku(tasks: list[OcrWorkerTask]) -> None:
    ...
    runtime_manifest = collect_runtime_manifest(
        "yomitoku",
        os.environ.get("OCR_MODEL_REVISION", "yomitoku"),
    )
    ...
    for task in tasks:
        ...
        try:
            ...
            primary_ocr_ms = round((time.perf_counter() - ocr_started) * 1000)
            full_text = "\n".join(item["text"] for item in results if item.get("text", "").strip())
            raw_output = json.dumps(results, ensure_ascii=False, default=str)
            result = SuryaPageResult(
                full_text=full_text,
            ...
            candidates = candidate_manifest(
                primary_text=result.full_text,
                primary_raw_output=result.raw_output,
                primary_state=result.state,
                primary_block_count=0,
                primary_quality_flags=result.quality_flags,
                primary_attempt_count=result.attempt_count,
                external_text=None,
