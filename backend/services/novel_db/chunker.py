"""ページ単位 / 書籍全体のテキストを embedding 用にチャンク分割する。

chunk_page()  : 1 ページの文字列 → チャンクリスト（後方互換、単体テスト用）
chunk_book()  : 全ページをまたいだクロスページチャンク（§4.4 新方式）

クロスページ方式のメリット:
- 小説 1 ページ（OCR）は 200〜600 字程度でページ末尾/先頭で意味が途切れる
- chunk_book() はページ境界を無視して 1200 字前後の意味的まとまりを生成する
- 各チャンクの page_id = **チャンク開始位置のページ** → リーダーへのジャンプに使う
