...
def upgrade() -> None:
    ...
    if "book_characters" not in existing:
        ...
        """))
        conn.execute(sa.text(
            "CREATE INDEX idx_book_characters_book ON book_characters(book_id)"
        ))
    ...
    if "qa_sessions" not in existing:
        ...
        """))
        conn.execute(sa.text(
            "CREATE INDEX idx_qa_sessions_started ON qa_sessions(started_at DESC)"
        ))
    ...
    if "qa_messages" not in existing:
        ...
        """))
        conn.execute(sa.text(
            "CREATE INDEX idx_qa_messages_session ON qa_messages(session_id, id)"
        ))
