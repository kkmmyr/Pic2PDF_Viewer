# 小説 RAG LLM バックエンド切替 設計案（実装済み・履歴用）

- **Status**: Implemented（2026-05-11、ADR-0009 採用フェーズ完了。本案の §3〜§5 は [小説テキスト検索・RAG機能_バックエンド設計.md §7](小説テキスト検索・RAG機能_バックエンド設計.md) にマージ済み）
- **Date**: 2026-05-11
...
> **NOTE**: 本ファイルは検討時のドラフト。実装後の正式仕様は本体設計書 §7 を参照。
> ここに残しているのは検討プロセス（採用候補の比較・テスト計画の詳細）の履歴として有用なため。
