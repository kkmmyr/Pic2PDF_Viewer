...
> status: frozen | archived: 2026-08-22 | 現行契約は [hitomi.la 新着監視設計書](../../design/詳細設計/機能別/hitomi新着監視設計書.md) を参照

この文書は、旧 API 詳細、外部 NOZOMI 仕様メモ、systemd 手順を含む整理前の設計・運用記録である。新しい仕様を追記しない。
...
- NOZOMI 取得が失敗した場合（ネットワーク障害等）は共通`get_logger`経由で
  consoleと`backend/data/logs/app.log`へ警告を記録するが、登録自体は成功扱いで返す。
  この場合は初回監視時に全件が新着として出現することがある
...
実装計画（Phase 1-4）・設計判断の経緯は [凍結記録](../hitomi監視_実装計画と設計判断.md) を参照。
