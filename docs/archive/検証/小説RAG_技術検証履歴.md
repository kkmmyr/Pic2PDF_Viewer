...
> frozen: 2026-08-22 | role: 時系列の実測・採否履歴
>
> 現行推奨構成は[小説RAG 現行技術知見](../../log/技術知見/小説RAG_技術知見.md)を参照する。
...
### 9.35 Nemotron 3.5 Lightningは短窓thinkingに強いが、長文事実抽出の根拠精度が不足する（2026-08-14）

新規候補`nemotron-3.5-lightning:30b-a3b-q4_K_M`をM1 Max 64GBへ隔離導入した。必要runtimeに
合わせてHomebrew Ollamaを0.30.11から0.32.9へ更新したが、既存Qwen、Gemma、Muse、bge-m3は
削除・上書きしていない。Ollama実測は32.9B、Q4_K_M、25GB、context 1,048,576、
completion・tools・thinking対応、model ID `e7a64ff15fb1`だった。配布CDNの接続リセット後は、
保存済み16GBを維持して未取得範囲だけを100MB分割で補完し、全体SHA-256がmanifestの
`5c19f6282f4fc51cb114cb6c876d70ca2fc3b9cf0fbd0a018d9908f4fe1f63b3`と一致してから登録した。

茉莉花官吏伝10巻を本番サーバーから読み取り専用で参照し、§9.34と同じ74〜75ページ固定ケースを
JSON Schema付きで評価した。directは13.752秒、34.578 token/sでJSON形式を守ったが、仁耀の
最終行動を牢へ戻ると誤判定し、同じ理由文の末尾では十年間逃亡を選んだと述べる自己矛盾も生じた。
thinkingありは108.496秒、thinking 10,172文字を使い、最長10年の逃亡継続、途中の牢へ戻る記録の
矛盾、牢へ戻ったという誤要約の矛盾を3件とも正しく判定した。意味判定にはthinkingが必要だった。

しかし8〜27ページの第1事実ブロックでは、thinkingありは約8分28秒後も必須`[BOOK_FACTS]`を返さず、
現行parserの機械ゲートに不合格となった。directは98.610秒、入力21,229 token、出力2,312 token、
23.546 token/sでページ付き事実2,941字を生成したが、先頭マーカーだけを欠落した。全非空行が
`- [page N]`で始まる場合に限る決定的アダプターでマーカーを補うと、人物別再編も145.695秒、
出力5,847 token、42.561 token/sで完走し、104 recordの参照ページはすべて入力8〜27内だった。

機械的な範囲内参照だけでは根拠の正しさを保証できない。原文抜き取り照合では、子星の発言を茉莉花へ
帰属する話者誤り、18〜19ページとは別の場面をそのページへ置く場面ずれ、26ページの命令を27ページの
事実として扱うずれを確認した。人物見出しは18件へ増え、既存正規化後も16件が残り、正規名の短縮、
役職別名、同一人物の重複が混在した。これはページ番号が許可範囲内という機械ゲートを通過しても、
完成要約の根拠として安全ではない失敗例である。

32,768 contextではモデル25GB・100% GPU、システム空きメモリ54%、swap in/out 0で、64GB Macへの
単独適合性は確認できた。品質優先の早期ゲート不合格により、残り3ブロック、詳細あらすじ、一覧要約、
人物辞典の生成には拡大していない。巻全文の主生成・事実抽出には採用せず、本番既定値へ配線しない。
短窓thinkingもMuseの同ケース45.63秒より遅く、現時点では任意の第二検証役としても明確な追加価値を
確認できない。比較用登録だけを残し、評価後はアンロードした。

### 9.36 Qwen3.8-27Bは抽出構造を部分改善するが、完成要約はQwen3.6を置換できない（2026-08-15）

`qwen3.8:27b`（27.3B、Q4_K_M、17GB、context 262,144）をOllama 0.32.12へ追加し、
現行基準の`qwen3.6:35b-a3b`と比較した。茉莉花官吏伝10巻の同じOCR 77ページ、
8〜27、28〜49、50〜69、70〜84ページの4ブロック、現行プロンプト、32,768 context、
seed 20260813を使い、Thinking影響を除くため両方とも`think=false`に固定した。本番DB、公開要約、
OCR本文、索引は更新していない。

74〜75ページの固定短窓は両方とも、仁耀が最終的に合意した行動を最長10年の逃亡継続、
旧事実メモと「最終的に牢へ戻った」要約を矛盾と正しく判定した。Qwen3.6 directは
15.818秒、40.896 token/s、Qwen3.8 directは54.893秒、8.553 token/sだった。過去の同固定事例での
Qwen3.6失敗は現行プロンプトでは再現せず、短窓判定だけでは世代差を示せなかった。

4ブロック抽出では、Qwen3.6は第3・第4の人物応答がそれぞれ8,192 tokenに達し、
同一文の反復と途中切れを起こした。Qwen3.8はこの反復崩壊を避け、人物集合も複数の正規名で
構造化できた。ただし第3ブロックに未入力の70〜71ページの具体的な後続事実を生成し、
許可外ページを含む6項目を事実内容を修正せず項目ごと除外してから厳密検証を通した。
Qwen3.6にも根拠のない「該当人物の記載はない」という免責文1項があり、同じ方針で項目除外した。

| 指標 | Qwen3.6-35B-A3B | Qwen3.8-27B |
|---|---:|---:|
| 4ブロック抽出 | 1,337.803秒 | 3,328.746秒 |
| 詳細・一覧生成と校正 | 543.771秒 | 2,503.474秒 |
| 合計 | 1,881.574秒（約31分22秒） | 5,832.220秒（約97分12秒） |
| 詳細 / 一覧文字数 | 1,206 / 551 | 6,514 / 596 |

Qwen3.8の詳細版は「長くて十年間逃げ続ける」と必要時の将来証言を保持し、この一点はQwen3.6より良い。
しかし初稿と校正版の両方が4,096 token上限に達して末尾を文途中で切り、仁耀の戦争計画を
珀陽の計画とする帰属誤りも含んだ。一覧版は400〜700字契約を守ったが、仁耀の殺害対象を
黒の皇帝ではなく珀陽と誤り、最長10年の逃亡と将来証言を落とした。Qwen3.6も黒の皇帝を
珀陽と同定し、一覧版で中間発言の「牢に戻る」を最終状態にした。どちらも機械的な文字数・形式ゲートを
通過しても公開できない。

ただし、途中切れはモデル能力だけではなく実行設定の上限にも起因する。Qwen3.8の詳細版は初稿・校正版が
ともに`num_predict=4096`へ到達した。Qwen3.6の校正版も入力32,012 tokenと出力756 tokenの合計が
`num_ctx=32768`と一致し、文途中で終了した。この2件は完了理由を保存していないため、次回はOllama応答の
`done_reason`を監査ログへ保存し、`length`終了をfail closedにする。詳細版と校正版は
`num_predict=8192`、校正は`num_ctx=65536`の候補を試し、出力上限の解消と事実精度を別々に判定する。

現行試験のsamplingは、Qwen3.8のローカルModelfileが持つ`top_p=0.95`、`top_k=20`、
`min_p=0`を使いつつ、用途別に`temperature=0.1〜0.2`、`repeat_penalty=1.15`へ上書きした。
Qwen3系の公式non-thinkingガイダンスは`temperature=0.7`、`top_p=0.8`、`top_k=20`、`min_p=0`を
例示しており、現行値はかなり保守的である。ただし事実抽出でtemperatureを上げると握造が増える可能性もあるため、
一括置換しない。保存済み事実表を使う要約工程だけで、次の順にA/B試験する。

1. 現行samplingのまま出力・context上限だけを拡大する。
2. `temperature=0.7`、`top_p=0.8`、`top_k=20`、`min_p=0`、`repeat_penalty=1.15`を比較する。
3. 2に人物・役割同定表、中間状態と最終状態の必須区別、未完文検出を加える。

詳細版はトークン上限だけで長さを制御せず、2,000〜4,000字を初回候補として、発端、主要対立、転機、結果、
最終状態を残せるかを評価する。事実抽出の許可外ページは、文字列マーカーだけでなくOllamaのJSON Schemaと
ブロック内page列挙で拘束する候補を試す。A/Bの最良条件が主体、殺害対象、最長10年の逃亡、将来証言、文末完結を
すべて通した場合だけ、4ブロック全体を再試験する。Q4_K_Mの設定改善後も品質不足の場合に限り、
より高精度な量子化を第二段階で検討する。

初回仮結論は、Qwen3.8を現行設定のままQwen3.6と置換しない、とする。モデル登録は設定再評価用に残すが、
本番既定値と自動公開へ配線しない。上記の局所A/Bが合格するまで、同条件の3回再現性試験と巻全体再生成は実施しない。

### 9.37 Qwen3.8は上限・sampling・役割表を調整しても公開品質へ到達しない（2026-08-15）

§9.36で保存した4ブロック事実表だけを使い、再抽出や本番DB更新を行わず3条件を局所比較した。
入力は許可外pageを含む6項目を除外した27,377文字で、SHA-256は
`43a3a4d51b03f31b34190ed1fb9949ae9a668c3f7ffa15a7787e32d5e844d9f0`である。
全条件で`qwen3.8:27b` Q4_K_M、`think=false`、seed 20260813を固定し、詳細初稿は
`num_ctx=32768 / num_predict=8192`、詳細校正は`num_ctx=65536 / num_predict=8192`、
一覧初稿・校正は`num_ctx=32768 / num_predict=1024`とした。

| 条件 | sampling / 追加契約 | 詳細 初稿 / 校正 | 一覧 初稿 / 校正 | 4呼出合計 | 判定 |
|---|---|---:|---:|---:|---|
| A | temperature 0.2/0.15、top_p 0.95、上限だけ拡張 | 8,663 / 8,664字 | 1,554 / 1,554字 | 3,159.789秒（約52分40秒） | 不合格 |
| B | temperature 0.7、top_p 0.8、top_k 20、min_p 0 | 9,067 / 9,133字 | 550 / 548字 | 3,055.708秒（約50分56秒） | 不合格 |
| C | B + 役割表 + 中間/最終状態 + 詳細2,000〜4,000字 | 1,207 / 1,398字 | 1,181 / 839字 | 1,478.394秒（約24分38秒） | 不合格 |

条件Aでは詳細初稿・校正が5,426 / 5,429 tokenで`done_reason=stop`となり、4,096 token時の
途中切れは解消した。しかし一覧初稿・校正はどちらも1,024 token、`done_reason=length`で終わり、
上限拡張だけでは詳細の冗長化と一覧の未完を同時に解消できなかった。

条件Bでは一覧が550 / 548字、全4呼出が`done_reason=stop`となり、公式non-thinking samplingは
短縮要約の長さ制御だけを改善した。一方、一覧から仁耀の黒の皇帝殺害、最長10年の逃亡、将来証言が
すべて落ちた。詳細も9,000字を超え、sampling変更だけを本番値へ採用できない。

条件Cは全4呼出を自然停止させ、条件Aより約53.2%短時間化した。最長10年の逃亡、将来の脱獄証言、
黒の皇帝という殺害対象も残したが、詳細は2,000字下限未達、一覧は700字上限超過となった。
さらに「大陸統一を目指す珀陽の構想」と書いて仁耀側の構想を再び珀陽へ帰属させ、保護作戦も
「仁耀を止めるためではなく」と変形した。これは正しい役割と最終状態をプロンプトへ明記しても残った
意味誤りであり、決定的な文字数・完結ゲートだけでは公開を許可できない。

3条件合計は7,693.891秒（約2時間8分14秒）。局所A/Bが1件も合格しなかったため、3回再現性試験、
巻全体再生成、より高精度な量子化の取得には進まない。Qwen3.8は比較用登録だけを残し、現行Qwen3.6、
本番既定値、自動公開を置換しない。追加検討する場合も、自由文への役割表追記ではなく、主語・行動・対象・
