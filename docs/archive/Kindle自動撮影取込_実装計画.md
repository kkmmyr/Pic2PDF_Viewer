...
状態: **Phase 5 実機受入・段階導入完了（2026-07-26）**

バックログ: [B-34](../log/計画/バックログ.md)

要件: [Kindle 自動撮影取込 要件](../design/要件定義/Kindle自動撮影取込_要件.md)

現行設計: [Kindle 購入カタログ設計](../design/詳細設計/機能別/Kindle購入カタログ設計.md)

運用・実機知見: [Kindle 自動撮影 実機知見](../log/技術知見/Kindle自動撮影_実機知見.md)
...
1. ライブラリ検索欄は UI Automation の `ControlType.Edit`、名前 `検索ライブラリ`
   として公開され、成立性調査では `ValuePattern.SetValue()` で ASIN を入力できた。
   ただし後続のクラッシュ実測により、現行仕様では `SetValue()` を使用しない。
...
10. 固定レイアウト漫画では `Home` と `Ctrl+G` のショートカット単独経路を
    先頭移動に使えなかったが、読書領域へフォーカスした後に `PageUp` を反復すると
    読書開始側へ戻れる。ツールバーの「位置に移動」からロケーションを指定する経路は、
    後続の10.7で別途成立を確認した。
...
- Microsoft Store版 Kindle 1.0.18632.0 の漫画読書画面では、
  `ReadingArea` は取得できるが、ツールバー非表示時は `FooterLabelText` が存在しない。
  2026-07-26の追試ではツールバー表示後に取得できたため、常時利用可能ではない要素として
  扱う。
...
- 固定 `PageUp` は右開き漫画を巻末へ進めるため、次ページ方向と逆の矢印キーで
  先頭へ戻す。撮影側も端部クリックではなく選択済み矢印キーを使う。
- 実機の画像差分は、本文ページ遷移が平均6.82、端部chevron表示だけの変化が0.38だった。
  平均差1.0未満を同一画面として、UI表示の微小変化をページ数へ加算しない。
- UI Automationの `SetValue` / `Click` 実行時にKindle 1.0.18632.0が
  `ucrtbase.dll / 0xc0000409` でクラッシュした。controlのASIN照合と矩形取得は維持し、
  実操作だけを矩形中心への通常のマウス／キーボード入力へ切り替える。

### 10.6 Phase 5 受入結果（2026-07-26）

対象アプリは Microsoft Store版 Kindle 1.0.18632.0。利用者指定の3冊を正式データとして
残し、失敗試験の一時成果物だけを破棄した。

| ケース | 結果 | 根拠 |
|---|---|---|
| ダウンロード済み comic | 合格 | `B08M8XZWM7`、job `559a9fc2-c69f-4a5d-b07a-188d8af00682`、168画面 |
| ダウンロード済み novel | 合格 | `B0DJ3DYD7M`、job `7dc73657-a2d4-4254-b1eb-5f52cd01dd64`、92画面 |
| 未ダウンロード novel | 合格 | `B0FTDGD2XV` の正式contentを一度除去し、再取得後にjob `9a5ad71e-2d5c-4ff3-ab5b-593647530b4f` で91画面 |
| 未ダウンロード comic | 共通経路で合格 | ダウンロードはcapturer選択前の共通controller処理。未ダウンロード実機とcomic撮影実機を独立に確認 |
| 読書途中から先頭へ移動 | 合格 | comic 168画面、novel 91画面で先頭境界から最終画面まで走査 |
| 類似タイトル・別巻 | 合格 | 実機のASIN付きcard完全一致と、候補なし・複数・ASIN不一致の単体テスト |
| Kindle 未起動 | 合格 | job `6a24a7b7-92ac-47b7-bb58-fecdb4181ad1` が `kindle_not_running` で安全停止 |
| ダウンロードタイムアウト | 合格（自動試験） | controllerの仮想時計テストで `download_timeout` と正式領域不変を確認 |
| agent異常終了・stale回収 | 合格（自動試験） | backendのheartbeat期限超過回収テストで `agent_heartbeat_timeout` を確認 |

- Samba受信箱のWindows書込・Linux読取・削除を実環境で確認した。
- 成功3jobは処理済みpackageと正式画像フォルダーの件数が一致する。
- デスクトップ1440×900、iPad相当768×1024、ライト／ダーク、各状態、キーボード操作を
  実ブラウザーで確認した。
- backend 982件（1 skip）、frontend 1,021件、kindle-pdf 32件のテスト、frontend型検査、
  lint、本番buildを完了した。既存のfrontend lint warning 18件は今回の変更外。

### 10.7 先頭移動の性能改善に関する追補（2026-07-26）

Phase 0 の初回調査では、固定レイアウト漫画で `Ctrl+G` を直接送る経路を
先頭移動に採用できず、画像差分を確認しながら1画面ずつ戻る方式を実装した。
その後のシリーズ連続取り込みで、読書位置が末尾に近い書籍では先頭移動だけで
数分を要することが顕在化した。

ツールバーを表示して `moreMenuButton` から「位置に移動」を選ぶ経路を再調査し、
小説・漫画とも `go-to-page-input` へロケーション `1` を通常のキーボード入力で
指定できることを確認した。小説は `Location 2748 of 3304` から
`Location 1 of 3304` へ約0.12秒、2ページ表示漫画は `Location 150 of 169` から
表紙の `Location 2 of 169` へ約0.17秒で移動した。

操作部品は共通だが、漫画はロケーション `1` を指定しても先頭表紙の表示値が
`Location 2` になる。このため実装時は `Location 1` 完全一致を共通条件にせず、
小説と漫画で先頭確認を分ける。`go_to_start(source, direction)` は高速経路を
優先し、通常操作のみを使う制約と、取得不能・検証不成立時の画像差分フォールバックを
維持する。
...
- [x] [要件定義の受入条件](../design/要件定義/Kindle自動撮影取込_要件.md)をすべて満たす。
- [x] backend、frontend、kindle-pdf の関連テスト、lint、型検査が成功する。
- [x] 実機マトリクスを完了し、対象 Kindle アプリのバージョンを記録する。
- [x] 現行設計書と API 仕様を実装済みの現在形へ更新する。
- [x] `docs/log/変更履歴.md` へ実装・実機受入結果を追記する。
