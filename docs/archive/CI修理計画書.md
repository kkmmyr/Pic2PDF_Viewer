...
**2026-07-18 実CI追補**: 初回PR実行で、ローカル環境差に隠れていた追加要因を確認した。frontend は npm 11 で生成した lockfile に npm 10 が要求する optional peer 情報（`@emnapi/core` / `@emnapi/runtime`）が不足していたため、CI と同じ npm 10.9.4 で再生成し npm 10 / 11 双方の `npm ci` を検証した。docs は `.coverage` に加え、ローカルにのみ存在する `backend/data/` / `backend/input/` がコミット済みツリーへ混入していたため、tracked source map の対象外へ統一した。backend の修正は初回実CIで green を確認済み。
