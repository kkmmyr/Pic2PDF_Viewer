...
2026-05-11 に r/LocalLLaMA で報告された **llama.cpp 直叩き + MoE block を `-ncmoe` で細かく制御 + KV cache を q8_0 量子化** という設定が、RTX 3060 12GB で同じ Qwen3.6-35B-A3B IQ4_XS を 46.8 t/s で動かすという内容だったため、本機での再現性を検証した（[小説RAG_技術知見.md §9 LLM 推論バックエンド検証](../../../log/技術知見/小説RAG_技術知見.md)）。
...
- KV cache q8_0 量子化により、本機の 12GB VRAM で **16k token context を保持しても tg 76 t/s 維持**（[Phase 2b depth スイープ](../../../log/技術知見/小説RAG_技術知見.md)）
...
- ロールバックは `NOVEL_DB_LLM_BACKEND=ollama` で 1 行（環境変数）で戻せる
  ...
  `ollama rm` で 23GB 解放。詳細は [LLM 層リファクタリング_完了記録 §5](../../../archive/LLM層リファクタリング_完了記録.md)）
...
採用最適設定の更新は [小説RAG_技術知見.md §9.3](../../../log/技術知見/小説RAG_技術知見.md) を参照（B-14 時点 / B-13 段階 C 本採用後 を併記）。
