...
以下の決定・比較は採用当時の記録である。現在の設定は `backend/config/novel_db.py`、共通クライアントはworkspace内の `common/llm/` が正本で、人物抽出モデルの既定値は `gemma4:12b`。現行の保存・生成契約は[小説RAGデータ設計](../../詳細設計/機能別/小説RAG_データ.md)を参照する。
...
| B. クラウド LLM（Claude / GPT-4 / Gemini）に切替 | 品質は確実に上がる | 個人 LAN ツールでクラウド送信は方針に反する（[セキュリティ設計書 §1](../../詳細設計/セキュリティ設計書.md)）。コストも常時稼働には不向き |
