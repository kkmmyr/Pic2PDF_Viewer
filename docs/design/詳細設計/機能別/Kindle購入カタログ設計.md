...
`/kindle/catalog` は、統計、購入書籍検索、種別・所有・画像状態フィルター、初回移行、3 系統の差分取込、既存画像の確認付き ASIN 紐付け、キャプチャジョブ作成・監視を 1 画面に集約する。

初回移行完了後の通常運用 UI は [Kindle 購入カタログ画面 UI/UX 改善 要件](../../要件定義/Kindle購入カタログ画面_UI改善_要件.md) に従い、購入書籍、画像紐付け、キャプチャ、取込・管理の 4 タブへ分離する。各タブは固有 URL を持ち、購入書籍を初期ページとする。
