...
> **Sol導入の境界**: B-36の全巻直接fact graph生成は不採用であり、現在のQwen経路を維持する。
> 再開には[Sol導入計画](../../../log/計画/Sol生成・評価_導入計画.md)に沿った別候補経路と受入判断が必要。
> 隔離CLIの契約は[Sol生成・評価設計](小説RAG_Sol生成・評価設計.md)を参照し、Sol候補を公開DBへ書き込まない。
...
- **比較モデルの採否**: ローカル系統の主生成器はQwenを維持し、公開成果物の主生成は
  [Sol生成・評価設計](小説RAG_Sol生成・評価設計.md)の契約に従う。導入の未完了作業と停止条件は
  [Sol生成・評価 導入計画](../../../log/計画/Sol生成・評価_導入計画.md)を参照する。比較結果から自動公開へ昇格しない。

  | 比較対象 | 利用・再評価の境界 |
  |---|---|
  | Gemma4 31B | 主生成・人物生成・既定検証役へ置換しない。Qwenをアンロード後、高リスクの少数主張を短い本文窓で直列検証する候補に限定する。巻全文一括検証や校正の自動確定には使わない |
  ...
  再評価では全ブロックの機械合格、正規名、最大期間・確信度・主体の意味ゲートを維持し、
  手動救済した結果を自動経路の合格へ数えない。
...
- **キャラクター関係グラフ（`relation_extractor.generate_book_relations`）**: `pages.main_characters`の同一ページ共起を数えエッジ重みとし、`book_characters.summary`をQwenへ渡して関係タイプをJSON抽出し、`character_relations`へREPLACEする。`mode=generate_relations`ジョブが生成を担当する。読み取りは`routers/novel_graph.py`から`graph_query`を呼び、series単位のnodes/edgesを返す。API境界は[データ設計 §4](小説RAG_データ.md#4-api)を参照する。
