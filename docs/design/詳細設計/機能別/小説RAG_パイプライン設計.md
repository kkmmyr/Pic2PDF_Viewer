...
- **Gemma4の位置付け**: 10巻の固定誤り1件では、Qwen3.6 35B-A3Bが仁耀の最終行動を
  「牢へ戻る」と誤判定した一方、Gemma4 12Bは「最長10年間逃げ続ける」を選び、誤要約を
  `contradicted`にできた。ただしJSONコードフェンス混入、事実メモ内部矛盾の見落とし、
  日本語校正時の意味変更も確認したため、既定検証モデルへは昇格しない。
  Qwen執筆後にモデルをアンロードし、Gemmaを独立した第二検証役として直列実行する候補とする。
  採用判断は、正解を固定した10〜20件以上で事実判定、出力契約、意味保存、所要時間を比較して行う。
