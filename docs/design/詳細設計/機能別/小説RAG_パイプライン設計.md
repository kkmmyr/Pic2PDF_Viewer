...
- **事実チェックポイント**: 各ブロックの抽出完了後、ページ範囲、本文SHA-256、
  ...
  `fact_extraction_blocks`へUPSERTして独立commitする。同じブロック番号でも本文ハッシュ、
  モデル、スキーマ版のいずれかが異なる場合は再抽出する。後段の執筆・校正が失敗しても
  一致する完成ブロックは次回再利用する。ブロック数が減った場合は余剰行を削除する。
- **ページ根拠検査**: 書籍事実と人物事実の全箇条書きに1件以上の`[page N]`を必須とし、
  指定ページがその入力ブロック内に存在しない出力は保存せず失敗とする。
...
- **Step 2d 人物別執筆**: 事実表の人物名を`character_names.normalize_character_entries`で正規化し、本文に根拠がある人物を登場ページ数順で最大20名選ぶ。人物ごとに関連ページと事実メモを渡し、他人物と混在させず個別に説明を生成する。
...
- **Step 2e 双方向RAG検証**: 校正後の詳細あらすじを句点単位の主張へ分解する。各主張について、
  ...
  近い事実の根拠ページを候補にする。候補ページを重複排除し、全主張、ページ本文、書籍事実を
...
- **検証fail closed**: 検証応答のJSON形式不正、主張IDの欠落・重複、候補外ページの引用、
...
- **機械品質ゲートの限界**: 文章形状検査とStep 2eは、完成文の主張根拠と重要事実の欠落を
  検査する。一方、同一人物の正規名が公開版と一致すること、
  既存人物の削除が妥当であることまでは保証しない。B-36で意味的根拠検査、
  正規名・別名統合、人物集合の削除回帰検査をすべて実装するまでは、機械ゲート通過後も
  全文差分のCodex補助QAを公開条件とする。
