...
文字位置コンセンサスは加重CER 0.44%を再現したが、ページ最大1.946%、固有名詞266/278で総合ゲートは`FAIL`である。候補別の詳細値は実装計画を正本とする。
Codex確定本文の0.00%は正解化実績であって独立したOCR性能値ではない。第三シリーズを加え、固有名詞台帳とページ単位昇格を固定してから正式holdoutを再評価する。
