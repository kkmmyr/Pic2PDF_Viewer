...
固定20画面はSHA再照合後に原画像と全文を比較し、132箇所を補正して正規化後27,629字をverified ground truthへ登録した。選定外ページとrun全体は`awaiting_qa`のまま公開しない。
文字位置コンセンサスは加重CER 0.44%だった。候補支持付き固有名詞11件の補正後は0.398%、ページ最大1.647%、固有名詞277/278で、機械単独は`FAIL`とする。
固有名詞台帳は対象画像・正解本文を含まないシリーズ単位で事前固定し、同位置の独立候補が正表記を支持する同長1文字置換だけを自動補正する。候補支持なし・競合・挿入削除は本文を変えずQAへ送る。
候補間最大正規化距離15%以上、または同一誤表記を2候補以上が支持する未解決台帳近似がある画面はCodex原画像QAへ昇格する。7画面を解決した補助込み候補はCER 0.23%、ページ最大0.714%、固有名詞278/278で合格したが、第三シリーズで条件を変えず再評価する。
