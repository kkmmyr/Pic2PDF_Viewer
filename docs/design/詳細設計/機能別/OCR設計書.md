...
学習・最終holdout未使用run 4 / 8 / 13から計60画面を系列内事前検証用に予約したが、同一シリーズ
3冊なので正式判定には使わない。原画像QA前はground truthへ登録せず、正式判定では3シリーズ以上、
固有名詞10語・50出現以上を画像SHAへ注釈する。非本文分類は別の固定コーパスで維持する。
この60画面では候補類似度0.94未満または8字以上の両側アンカーを持つ補完提案がある19画面を
Codex画像QA優先対象とした。類似度閾値と単一候補の補完は診断用で、自動確定には使わない。
初回確認3画面はいずれも実欠落で、primary／externalが別々の列を落とす例もあった。
`prepare_ocr_prevalidation_qa.py`の`--verify-queue`は優先19画面の件数とrun・ページ・画像SHA集合だけを照合する。
元の60画面は選択manifestで固定するが、パッケージ全体のdigest検査は未実装であり、不一致はSolへ戻す。
