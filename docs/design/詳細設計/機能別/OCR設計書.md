...
- 全ページへ決定論的なページ種別候補を設定する。目次は前付け位置と複数章見出しを
  ...
  種別を上書きしない。
- 現行のページ種別提案はページ単位の決定論的ヒューリスティクスであり、1つの配信書籍に
  本編と別作品試し読みが連結された場合の「書籍境界」は自動確定しない。2026-07-28の
  茉莉花官吏伝1〜7巻QAでは、本編後の第2書名・第2目次・別作品人物紹介を境界根拠として
  3・5・6・7巻の試し読みを`colophon_or_ad`へ確定した。実装で境界検出を追加するまでは、
  `あとがき`以後に第2の書名、目次、人物紹介、`試し読み`が現れたrunを全件QAし、
  境界以後を画像保持・索引対象外にする。
