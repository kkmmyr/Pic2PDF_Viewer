...
学習・評価未使用run 4 / 8 / 13の60画面を系列内事前検証へ予約する。候補類似度0.94未満または
両側8字アンカー付き補完提案がある19画面をCodex画像QA優先対象とするが、候補長や単一候補で
自動確定しない。`--verify-queue`は優先集合だけを検査するため、全60画面digestの不一致はSolへ戻す。
正式判定では3シリーズ以上、固有名詞10語・50出現以上を画像SHAへ注釈する。
未学習の別シリーズrun 70 / 71は各10画面を候補品質によらず等間隔抽出し、選定JSONへ固定する。
文字単位画像QA・補正文・固有名詞注釈前はground truthへseedせず、runも承認・公開しない。
primaryとexternalが別々の列を欠落し得るため、候補一致や単一アンカーを正解保証にしない。
