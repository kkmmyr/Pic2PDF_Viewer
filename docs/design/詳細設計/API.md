...
価格監視APIは `kindle_catalog.db` の監視対象と観測履歴を管理する。価格を取得する責務は
APIではなくCodexブラウザ観測にあり、APIはCodexまたは画面から渡された観測値を検証して
保存するだけである。現在価格または定価が欠けた観測は、該当する通知条件を評価しない。

詳細なテーブル、観測JSON、失敗時挙動は [Kindle価格監視設計](機能別/Kindle価格監視設計.md)
を参照する。
