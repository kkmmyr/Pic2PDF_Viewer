...
**Date**: 2026-05-11 / **関連**: [ADR-0009](../02_基本設計/ADR/0009_llm-backend-llama-server.md) / [バックログ B-14](../01_要件定義/バックログ.md) / [LLM 層リファクタリング_完了記録](../99_アーカイブ/LLM層リファクタリング_完了記録.md)（A〜C 完了で Backend ABC 抽象化 + Ollama rollback 撤去）
...
| **24（Ollama 非同時稼働時推奨）** | **未計測** | **50.2** | **60.8** | **59.7** |

**2026-05-12 追加 — ncmoe 26 / 24 実測（B-13 最適化検証）**:
- ncmoe 24 は 28 比 **A_short +8.4% / B_mid +9.4% / C_long +6.2%** の改善
- VRAM は未計測（ncmoe 24 で OOM なく完走したため少なくとも 12.2 GB 内に収まる）
- **gemma4:e4b（Ollama）と同時稼働しないシナリオ**（夜間バッチ後の QA のみ等）では `-ncmoe 24` を推奨
- gemma4:e4b が常駐する Query Expansion 同時実行シナリオでは VRAM 実測後に判断。当面は `-ncmoe 28` を本番設定として維持
