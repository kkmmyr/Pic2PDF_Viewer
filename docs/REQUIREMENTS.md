...
*   **Kindleキャプチャツール連動 (`kindle-pdf`)**:
    ...
    *   **小説 (`main_novel.py`)**:
        *   **白背景検知**: 白背景内のテキスト領域を自動検出してクロップ。
        *   **OCR文字認識**: `yomitoku` を利用して画像をテキスト化（`.txt` 出力）。
        *   **読み順補正**: 段落検出が不安定な場合でも、縦書きの読み順（右→左）でテキストをソートする robust なロジックを搭載。
        *   **フリガナ除去**: 文字サイズによるフィルタリングを行い、本文のみを抽出する。
    *   **共通機能**:
        *   **ページめくり方向指定**:
            *   撮影開始時に「左キー（右開き）」または「右キー（左開き）」を選択可能。
        *   **高性能化オプション**:
            *   OCR処理用にGPU版 PyTorch 環境を利用可能。
            *   `batch_ocr.py` による事後OCR処理（バッチ処理）をサポート。

## 3. 今後の検討事項 (Future Roadmap)

### 3.1. テキスト付きPDF (Searchable PDF)
*   **目的**: 画像ベースのPDFではなく、テキスト検索やコピーが可能なPDFを生成する。
*   **アプローチ**:
    *   OCRで取得したテキストと座標情報を使用し、PDF上に透明テキストレイヤーとして埋め込む（`sandwich PDF`）。
    *   `yomitoku` の出力結果を PDF 生成フローに統合する。
