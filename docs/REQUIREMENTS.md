...
## 3. 今後の検討事項 (Future Roadmap)

### 3.1. (実装済み) テキスト付きPDF (Searchable PDF)
*   **機能**: 画像ベースのPDF上に透明テキストレイヤーを埋め込み、テキスト検索やコピーを可能にする。
*   **実装**: `yomitoku` (OCR) + `ReportLab` (PDF生成) を連携させ、Kindle小説取り込みの標準フローとして統合済み。
