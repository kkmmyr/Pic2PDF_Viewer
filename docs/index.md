...
`docs/**/*.md` または `mkdocs.yml` を変更するコミットは、pre-commit フック経由で `scripts/maintenance/check_docs.py` が検査する。違反があるとコミットがブロックされる：

1. **リンク切れ** — docs 間の相対 Markdown リンクが実在ファイルを指しているか
2. **変更履歴の肥大化** — `log/変更履歴.md` が上限行数を超えていないか（超過時は週次ローテーションを促す）
3. **nav 同期** — `mkdocs.yml` の nav ツリーが実ファイルと一致しているか（dead entry / orphan ファイルがないか）
