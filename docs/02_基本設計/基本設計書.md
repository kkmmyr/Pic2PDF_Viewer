...
*   **Web UI Viewer**:
    ...
    *   **書籍メタデータ管理**: 作者名・タグ・シリーズ情報をサーバー側 JSON に保存。タイトル検索欄での横断検索・各種フィルター・複数書籍への一括設定（作者・タグ・シリーズ登録）に対応。**一括操作の成功時は選択モードを自動解除** して通常表示に戻す。
    *   **シリーズ管理**: 書籍をシリーズで集約・ドリルダウン表示。シリーズ内の巻順は **DnD で並べ替え** 可能（ドロップ即時保存・失敗時ロールバック）。シリーズ巻の閲覧中、最終ページ到達時に画面下部に **「次の巻へ」フローティングボタン** を表示し、同フォルダ内の次巻に 1 ページ目から遷移できる。
