...
- **Date**: 2026-05-10
- **決定者**: プロジェクトオーナー
- **関連**: [小説テキスト検索・RAG機能_バックエンド設計.md §1.2 / §2.2 / §7](../../03_詳細設計/小説テキスト検索・RAG機能_バックエンド設計.md) / `D:\61.tool\common\Qwen` / commit 履歴 2026-05-09 〜 2026-05-10
...
- **パス更新注記**: 2026-05-11 の A-0（[LLM 層リファクタリング計画](../../06_リファクタリング/LLM層リファクタリング計画.md)）で共通モジュールを `D:\61.tool\common\Qwen\` → `D:\61.tool\common\llm\` にリネーム、`qwen_client.py` → `local_llm/` パッケージに再構成。本 ADR 内のパス・API 名は決定時の歴史的記録としてそのまま残置
