...
| D. Qwen3.x 系（Qwen3.6:35b-a3b、35B 総 / 活性 3B MoE） | 35B クラスで活性 3B のため VRAM ~22GB に収まる | （採用） |
...
- **VRAM 22GB を消費**。同時に他の重い GPU タスクは実行不可（builder.py の embedding と LLM 質問応答も直列実行する制約は元々あり、影響軽微）
