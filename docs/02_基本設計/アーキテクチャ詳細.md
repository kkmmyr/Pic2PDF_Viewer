...
| [backend/routers/series.py](../../backend/routers/series.py) | シリーズ判定ジョブの実行・進捗、シリーズ手動割り当て / 解除 API |
...
| [frontend/src/components/viewer/LibraryDialogs.tsx](../../frontend/src/components/viewer/LibraryDialogs.tsx) | ライブラリ画面のダイアログを集約（CreateFolder / Rename / Move / BulkAuthor / BulkTag / Merge） |
| [frontend/src/components/viewer/ReaderPanel.tsx](../../frontend/src/components/viewer/ReaderPanel.tsx) | PDFリーダー UI（ページ表示・見開き制御・検索・削除・フルスクリーン・ショートカット） |
...
| [frontend/src/components/reader/LibraryHeader.tsx](../../frontend/src/components/reader/LibraryHeader.tsx) | ライブラリヘッダー（検索・ソート・作者/タグフィルター・グループ化セレクト・非表示トグル・シリーズチップ・選択モード） |
| [frontend/src/components/reader/PdfGrid.tsx](../../frontend/src/components/reader/PdfGrid.tsx) | 書籍カードグリッド（サムネイル・作者・タグ・シリーズ/作者集約バッジ・非表示/シリーズ編集ボタン） |
...
| ファイル | 役割 |
|---|---|
| [frontend/src/hooks/useBookMeta.ts](../../frontend/src/hooks/useBookMeta.ts) | 書籍メタデータ取得・更新（作者 / タグ / 非表示 / シリーズ手動編集 / 閲覧記録）。`allAuthors` / `allTags` / `allSeries` 集計 |
