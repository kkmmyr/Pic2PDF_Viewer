...
### 9.18 配布ONNXからPARSeq互換checkpointを復元できても、ページ品質は別に測る（2026-08-01）

NDLOCR-Lite配布物には参照先の`parseq-tiny`定義と初期checkpointが含まれていなかった。
標準PARSeqを初期値なしで学習したpilotは完全一致0%まで崩壊したが、上流Tiny定義と配布ONNXの
initializerを対応付けることで、175 tensor・8,831,654 parameterの互換state dictを復元できた。
復元物は全parameterの一意対応・形状・有限値を検査し、固定乱数と32実画像の3,333 token位置で
ONNX Runtimeとのtop-1不一致0件を確認した後にだけ学習へ使用した。

49,741行での追加学習は検証完全一致89.4072%、NED 99.4649%まで改善し、再変換ONNXも
top-1一致を維持した。しかし未調整holdoutのCERは3.2843%から2.6682%への改善に留まり、
目標0.5%へ届かなかった。行認識器のvalidation改善は、列検出漏れ、列順序、ページ結合、約物、
固有名詞を含むページ品質の代替指標にはならない。モデル差し替えはONNX同値性だけでなく、
未学習runのページCER・最大CER・列欠落・固有名詞ゲートをすべて通過した場合に限る。

開封済みholdoutで列gapから再生成したcropは32画面でCER 2.2758%まで改善したが、同じ標本を見て
濃色画素閾値を定めたため診断値である。次の判定では、新規2シリーズ以上を含む未調整holdoutを
先に固定し、閾値を変更せず再現する。候補間の文字列差はQA優先順位には使えるが、長い候補や
単一候補の挿入を正解とみなさず、原画像による文字単位確認を残す。

なお、現行の汎用品質policyはB-35固有の3シリーズ条件を持たず、holdoutの再開封拒否もCLIでは
未強制である。要件を文書に書くだけでは再利用時の事故を防げないため、B-35完了前にシリーズ数、
開封済み台帳、事前検証パッケージ全体のdigestを機械的なfail-closed条件へ昇格する。

### 9.19 新規シリーズholdoutは候補品質を見ずに先に画像SHAを固定する（2026-08-01）

追加学習や既存holdoutの診断に使っていない別シリーズとして、『悪役令嬢レベル99』と
『後宮一番の悪女』を購入済みカタログから取得した。OCR run 70 / 71は公開せず`awaiting_qa`で停止し、
通常散文ページの全域から等間隔に各10画面を抽出した。候補類似度、文字数差、QA要否は抽出条件に
使わず、選定後にrun・ページ・画像SHAを固定した。これにより、OCRが一致した容易なページだけを
選ぶ評価バイアスを避ける。

最初のrun 70・10画面は、primaryが冒頭列と中盤列を落とし、別段落を反復していた。一方でexternalも
行末欠落、類似漢字、約物、固有名詞に誤りがあり、長い候補を選ぶだけでは正解にならなかった。
正式ground truthは、各原画像と候補差分を文字単位で確認し、`selected_engine=codex`の補正文を
確定した後にだけseedする。run 4 / 8 / 13の茉莉花官吏伝予約群を第三シリーズに使うが、
新規20画面を含む全選定集合のdigestと開封台帳を機械検査できるまでは最終合否を出さない。

その後、run 70 / 71の固定20画面を原画像と全文照合し、132箇所を補正して正規化後27,629字の
verified ground truthを確定した。候補差分だけでは見落としやすい事例として、run 71・69画面では
「だから、琳麗は誘拐されても焦ったり、怖がったりはしない。」という一文全体の欠落を原画像から
回収した。候補間の多数決では双方にない本文を復元できないため、欠落疑いがなくても新規holdoutの
正解化には原画像確認が必要である。

同じ20画面で、primary 3.18%、external 2.24%、配布NDLOCR 1.38%、追加学習NDLOCR 0.81%、
文字位置コンセンサス0.44%となった。コンセンサスは集計CER 0.5%以下へ初めて到達した一方、
最悪ページは1.946%、固有名詞は278出現中12件を誤った。候補間で共通する人名・地名誤りは多数決で
残るため、固有名詞台帳による決定的補正と、ページCER上限を代理する昇格判定を別工程にする。
Codex確定本文のCER 0.00%は正解本文自身との比較であり、機械候補の性能には数えない。

ground truth登録後の確認では、一覧取得と単件更新APIが全登録本文のLevenshtein距離をPythonの
二次元DPで毎回再計算し、現コーパス規模で60秒を超えることがあった。登録自体の成功とAPI応答の
完了を混同せず、DB件数・verified状態を読み取り専用で確認する。APIの増分集計・キャッシュ化は
OCR品質変更と分離した技術メンテナンスとして扱う。

### 9.20 固有名詞台帳は正字辞書ではなく候補支持の制約として使う（2026-08-01）

新規2シリーズの文字位置コンセンサスでは、12件の固有名詞欠落のうち11件について、primaryまたは
externalが同じ位置で正しい表記を出していた。そこでシリーズ台帳を無条件置換に使わず、同長1文字差、
同位置の独立候補による正表記支持、非競合という条件をすべて満たす場合だけ置換した。これにより
ground truthを選択へ使わず、CER 0.44%から0.398%、固有名詞266/278から277/278へ改善した。

候補支持がない近似は自動修正しない。「董華→菫華」は実際の残差だった一方、「賢妃→徳妃」や
「リッ→リタ」のように短い語では正しい別語・通常断片も近似になる。これらをまとめて原画像QAへ
昇格すると7/20画面となった。誤検知は処理量を増やすが本文を壊さず、7画面をCodex確定本文へ置換した
運用候補はCER 0.23%、ページ最大0.714%、固有名詞278/278で合格した。機械候補の不合格と
Codex補助込み候補の合格を別レポートに保ち、後者を純機械性能と呼ばないことが重要である。

### 9.21 低い候補間距離は共通系統誤りの安全性を保証しない（2026-08-01）

未使用の第三シリーズとして、Windows版Kindleで取得できる講談社タイガ版
『虚構推理短編集　岩永琴子の密室』83画面を撮影した。候補本文を見る前に通常散文10画面を
等間隔に選び、画像SHAを固定した。原画像QAでは、8画面の本文列欠落、37画面の段落欠落、
52画面の本文欠落と段落・会話順崩れ、60画面の重複断片、73画面の`べール`反復などを確認した。
10画面すべてを原画像と全文照合して42の補正区間を確定し、20,667字のverified ground truthとした。
run全体は`awaiting_qa`・非公開のままで、選定外画面を承認・公開していない。

第三シリーズ10画面では、配布NDLOCR-LiteがCER 1.485%、追加学習版が1.214%となり、追加学習の
改善効果は別シリーズにも再現した。ただし単独0.5%には届かない。先行2シリーズと合わせた
30画面・48,296字ではprimary 2.437%、external 2.050%、4候補の文字位置合議＋候補支持付き
固有名詞補正が0.474%だった。集計CERは0.5%以下だが、ページ最大3.470%と固有名詞529/536により
総合ゲートは不合格である。特にrun 72・73画面の候補間最大距離は6.04%で、事前固定した15%昇格
閾値では検出できなかった。複数候補が同じ表記を誤ると、合意の強さは正しさの根拠にならない。

固定30画面をすべてSolで原画像照合した運用結果は、CER 0.000%、ページ最大0.000%、列欠落0、
固有名詞536/536でゲートに合格した。この0%は、原画像QAで作った本文を同じ本文から作成した
ground truthと比較した正解化実績であり、未知ページへの機械性能やSolの独立再現率ではない。
したがってB-35は「Codex補助込みの通常散文運用は合格、機械単独とSol省略は未達」と扱う。
低距離共通誤りの検出器が別の未調整標本で検証されるまでは、候補距離が小さくても自動公開せず、
固有名詞・片仮名／平仮名・反復語・列欠落・段落順を原画像QAの確認項目に残す。
