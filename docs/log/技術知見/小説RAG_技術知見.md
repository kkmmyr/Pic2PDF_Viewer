...
### 9.14 入力画像の途中終了と通知UI混入はOCR run承認だけでは防げない（2026-08-01）

46冊の公開後監査で、グリムコネクト1〜3巻の旧入力が20・18・14画面しかなく、
3冊とも本文途中で終了していたことを確認した。元画像を直接開くと通常の縦書き
リフロー小説であり、固定レイアウトではなかった。撮影側の開始位置、フォーカス、疎な
文字画面の差分判定が複合して短い入力を生成し、OCR側は渡された入力内だけを正常に
処理・承認していた。修正後の再撮影は1〜3巻が168・168・141画面で、全件の連番・復号・
寸法と、重複・白紙候補、先頭・中間・末尾を画像で確認した。重複候補は章間の
「次のページへ」、白紙候補は扉・著作権表記・疎な本文・奥付であり、欠落ではなかった。

この事例から、`completed / approved`は入力範囲内のOCR品質を表すだけで、書籍全体の
完全性を表さない。撮影完了とOCR承認を別ゲートにし、OCR前にKindle終端、画面数の妥当性、
先頭・中間・末尾の目視確認を必須とする。リフロー小説の紙面ページ数は撮影画面数と
一致しないため、既知の紙面ページ数を`expected_screens`へ流用しない。

別件として、ふつつかな悪女2巻の撮影画面47〜49へCodex / ChatGPT通知が重なり、
47・48画面の公開本文に通知文字列が混入していた。全公開本文を`Codex`、`ChatGPT`、
`Kindleカタログ`、`UI改善`、`要件整理`等で再検索したところ、実際のUI混入はこの3画面に
限定された。「通知」「完了しました」は正当な小説本文にも現れるため、一般語の単独一致を
自動判定へ使わない。強い製品名、または複数のUI固有語の組み合わせだけをQA必須flagへ送り、
原画像で本文との境界を確認してからUI文字列だけを除去する。

---

### 9.15 `image_only`は固定レイアウト判定ではなく、OCR候補は原画像の代替にならない（2026-08-01）

グリムコネクト1〜3巻を再撮影・再OCRし、run 67・68・69として公開した。3冊とも通常の
縦書きリフロー小説であり、再撮影画面数は168・168・141だった。3巻141画面を接触シートと
原解像度画像で全件確認し、本文112画面、目次1画面、表紙・扉・挿絵・前付・奥付・広告等
28画面へ分類した。機械分類で`image_only`だった93・106・129画面は通常本文であり、
`normal_prose`へ修正した。

ここで`layout_type=image_only`は「その画面では文字OCRより画像保持を優先する」という
ページ単位の判断であり、Kindle書籍全体が固定レイアウトであることを意味しない。
書籍レイアウトは連続ページの表示、文字サイズ変更可能性、位置・ページ表示、ページ送りの
挙動から判定する。扉・挿絵・前付が多い通常のリフロー小説を、個別ページの分類だけで
固定レイアウト扱いしてはならない。

3巻105画面では、主系候補は補助候補より長かったが、黒四角55文字、本文欠落、文字崩れを
含んでいた。補助候補は本文形状をより保っていたものの一部の行末が欠けていたため、どちらも
そのまま正解にできなかった。原画像を直接読み、補助候補を基礎に欠落箇所を復元して
Codex補正文として公開した。候補の長さは列欠落の発見には有効だが、長い候補ほど正しいという
採用規則にはできない。主系・補助系が異なる列を欠く場合もあるため、疑義画面は候補差分と
原画像を同時に確認する必要がある。

その後、4シリーズ46冊を対象集合固定で再監査し、原画像確認済み41画面と上記105画面を
SQLite Online Backup後に修復した。最終監査では全46冊を公開済みとして確認し、
`PRAGMA integrity_check=ok`、ページ件数不一致、公開本文不一致、ページ種別・索引可否矛盾、
補正文採用元矛盾、UI混入、隣接完全重複を各0件とした。残った反復4件、疎な本文14件、
候補欠落リスク3件、重複行28件は、原画像またはページ文脈で意図的な台詞・叫び、短い章末、
人物紹介・アンケート・前付等の索引対象外ページと確認した。これは文字単位の完全正解を
証明するものではないが、既知の構造欠陥と公開不整合を解消し、残余flagを説明可能にした状態である。

---

### 9.16 CERだけでは停止条件にならないためB-35を複合ゲート化した（2026-08-01）

固定20画面にはCER集計が既にあったが、数値を表示するだけで合否や終了コードを返さず、
列欠落と固有名詞をCERから独立して判定できなかった。そこでコーパス構成、通常散文の
加重CERとページ最大CER、Levenshtein最小編集経路の削除量による列欠落疑い、
画像SHAで固定した固有名詞の完全一致出現回数を別々の必須ゲートにした。

現行機械候補の初回結果は、通常散文加重CER 1.82%（基準0.5%以下）、ページ最大CER
2.07%（基準1.0%以下）で、4/4画面がページ上限を超えた。列欠落疑いはrun 1の
8画面が316字・18.65%、38画面が775字・94.40%の削除となり、既知の全幅要約と
本文＋挿絵の欠落を2/2件検出した。一方、通常散文4画面に固定した固有名詞10表記は
121/121出現を完全一致で再現した。

この結果から、固有名詞標本の合格だけでも、加重CERの平均だけでも自動公開可否を決められない。
特にページ最大CERは、ページ単位の劣化が長文ページに平均化されることを防ぐ。列欠落疑いは
40字・5%という安全側の機械flagであり、短い列や両候補共通欠落の完全検出を保証しないため、
最終確認は原画像を正本とする。現行機械候補は複合ゲート`FAIL`であり、Codex補助QAを
外す根拠にはならない。

---

### 9.17 LunaはOCR正本ではなく、Solへ渡す差分提案の作成に向く（2026-08-01）

ChatGPT認証のCodex CLIから`gpt-5.6-luna`を非対話・read-only・ephemeralで実行し、
B-35の通常散文entry 9（正解918字）を代表評価した。APIキー関連環境変数は未設定で、
`codex login status`が`Logged in using ChatGPT`であることを確認してから実行した。

原画像をlowで転記するとCER 8.39%・53字削除となり、冒頭列を落とした。左側の大きな余白を
除いて本文領域だけを渡したmediumは、説明の定型前置きを除外するとCER 1.63%・4字削除まで
改善し、指定固有名詞は完全一致した。機械候補を併用した出力はCER 1.20%・2字削除だったが、
0.5%基準には届かなかった。highは4分でタイムアウトし、完成本文を返さなかった。

候補併用では`呼ばう`→`呼ぼう`、`そうしょう`→`そうしよう`等を見つけた一方、原画像の
心内語括弧を別種の括弧へ変える誤提案もあった。固有名詞が合っていること、現行候補より
CERが低いこと、修正理由を説明できることのいずれも、Luna単独確定の根拠にはならない。
Lunaは差分要約・リスク順位・修正案を作り、Solまたは人が原画像で採否を決める役割に限定する。

実行経路にも注意が必要だった。PowerShell `Invoke-RestMethod`で取得した日本語JSONをそのまま
UTF-8ファイルへ保存すると、応答の文字コード解釈によっては二重文字化した。候補はバイト列を
UTF-8 JSONとして明示的に復号し、投入前後で文字列・SHAを照合する。長い候補はWindowsの
コマンド引数へ直接埋め込まず、検証済みUTF-8ファイルまたは構造化入力で渡す。

サブスクリプション利用ではAPIの価格改定を1ページあたり費用へ換算しない。今回のCLI表示は
medium単独転記で約16,000 reported tokens、候補照合で約30,000 reported tokensとなった。
これはAPI請求額ではないが、利用枠と処理時間を圧迫する指標にはなる。対象を既存リスクflagで
絞り、初期上限を1冊5画面とし、モデル・プロンプト・CLI・切り出し方式ごとに成功率と所要時間を
記録する。

Sol確認を省略するには、代表1画面の改善では足りない。固定20画面、未調整holdout、3回反復、
100画面連続試験を通し、CER、列欠落、固有名詞、無断意味変更、出力契約、タイムアウトを
独立ゲートにする。解禁後も10%をSolで無作為監査し、重大な見逃し1件で全件確認へ戻す。
