...
運用・実機知見: [Kindle 自動撮影 実機知見](../技術知見/Kindle自動撮影_実機知見.md)
...
小説と漫画で先頭確認を分ける。現在の `go_to_start()` は未変更であり、高速経路、
通常操作のみを使う制約、取得不能時の画像差分フォールバックは
[リファクタリング計画書](リファクタリング計画書.md)のP1技術メンテナンス候補として
実装・再受入を管理する。
