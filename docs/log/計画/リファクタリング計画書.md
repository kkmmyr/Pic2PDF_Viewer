...
最終更新: 2026-09-08（N5bを受入し、次のN6と補償失敗対策TM-11を整理）

**現在地: N0〜N5のローカル実装・検証は完了。N6〜N7は未着手。TM-8は環境待ち、TM-9/TM-11は未着手。**
...
[N5b記録](../../archive/検証/リファクタリング_N5b_2026-09-08.md)へ移した。
...
新計画はN系列で区別する。**下表に残すN6以降は未着手**。N0〜N5は受入記録へ移動済み。
...
残る推奨順はN6→N7。N6は受入済みN2を前提に独立して進められるが、
...
次の単位は**N6: Library/Readerの状態契約と機能入口の整理**とする。

1. `ViewerPage.tsx`、`useUrlState`、`libraryStore`、`useReaderState`の既存テストを確認し、
   URL戻る/進む、source/path同期、書籍切替時の初期ページ・前書籍stateリセットを先に固定する。
   一覧はreader表示中もmountを維持して隠す現行方式であり、scroll保持も比較対象にする。
2. URL、server cache、読書session、一時UIの書込み元と同期順を整理し、一画面・一状態単位で
   機能公開入口を絞る。既存の専門hookを再分割することや、一括ファイル移動を目的にしない。
3. query key・無効化、既読/再開、編集・見開き・一覧への復帰を比較し、変更する単位ごとに
   既存入口を保ったまま必要な表示/model/APIだけを集約する。
4. CI指定Node.js 22.23.2 / npm 10.9.8でfrontend検査を実施する。UI影響がある単位は
   desktop/390px、keyboard、console、同じ操作の再実行を受入に含める。
