...
最終更新: 2026-09-12（TM-9をmasterへ取り込み、TM-11の撮影補償と復旧再開をローカル受入）

**現在地: N0〜N7cとTM-9はmaster取り込み済み（TM-9はPR #24、6034fcf）。TM-11はローカル実装・検証完了（未commit）。追加の物理移動は見送り、互換撤去は外部確認待ち。TM-8は環境待ち。**
...
N0〜N6はPR #22でmasterへ取り込んだ。本番配備・実環境の追加受入は含まない。
...
TM-9もPR #24で取り込み済み。次はTM-11成果をcommit・push・PR検査・取り込みで区切る。
採用済みの実装作業はローカル受入まで完了し、その後はTM-8・互換撤去の外部確認と配備時の受入を進める。
