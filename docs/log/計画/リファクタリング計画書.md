...
最終更新: 2026-09-12（N7cの配置整理を受入し、追加の物理移動を見送り）

**現在地: N0〜N6はmaster取り込み済み。N7a/N7bはcommit済み、N7cはローカル実装・検証完了（未commit）。追加の物理移動は見送り、互換撤去は外部確認待ち。TM-8は環境待ち、TM-9/TM-11は未着手。**
...
新計画はN系列で区別する。**採用済みN0〜N7cのローカル実装・検証は完了。検索・全文索引・CLIの追加物理移動は今回は採用しない**。
ownerはproject owner、設計判断・統合・最終検証はメイン担当が行う。

| 残る単位 | 実施内容 | 完了条件・再開条件 |
|---|---|---|
| N7成果の確定・共有 | N7cを独立commitにし、N7a〜N7cをpush・PR検査・master取り込みへ進める | 最新headのCI成功、取り込み確認、計画と履歴の状態更新。本番配備は別途扱う |
...
次は**N7c成果のcommitと、N7a〜N7cの共有・取り込み**で区切る。
未共有の範囲を確定してから、新たな実装作業はTM-9/TM-11等の失敗時の振る舞い改善を別単位で検討する。
