...
最終更新: 2026-09-08（N4を受入し、次のN5aとTM-10の基準値整理を具体化）

**現在地: N0〜N4のローカル実装・検証は完了。N5〜N7は未着手。TM-8は環境待ち、TM-9/TM-10は未着手。**
完了範囲と検証は[N0〜N2記録](../../archive/検証/リファクタリング_N0-N2_2026-09-08.md)と
[N3記録](../../archive/検証/リファクタリング_N3_2026-09-08.md)、
[N4記録](../../archive/検証/リファクタリング_N4_2026-09-08.md)へ移した。
...
### TM-10. 検索分離で不要になった基準値の整理 — 未着手

- **現在地**: N4の構造検査で`search.py`が462行から287行となり、`hybrid_search`のC901超過も解消した。
  監査scriptは基準値の削除を独立した保守変更として行うよう案内しているため、N4ではbaselineを変更していない。
- **次の一手**: `code_size_baseline.json`の`backend/services/novel_db/search.py`と、
  `c901_baseline.json`の`backend/services/novel_db/search.py:hybrid_search`だけを実測値に照合して削除する。
- **完了条件**: 規模・C901・保守資産の各検査が合格し、新しい超過を基準へ追加せずに旧例外2件を撤去できること。
- **owner / 再評価条件**: project owner。次回の保守コミットで実施する。N5aの着手を妨げない。
...
[N4受入記録](../../archive/検証/リファクタリング_N4_2026-09-08.md)を参照する。
...
新計画はN系列で区別する。**下表に残すN5以降は未着手**。N0〜N4は受入記録へ移動済み。
ownerはproject owner、設計判断・統合・最終検証はメイン担当が行う。
各段階はさらに独立して検査が通る小単位へ分け、前段の受入後に次の範囲を確定する。

| 段階 | 実施内容・主な所有範囲 | 受入条件 | 依存・優先度 |
|---|---|---|---|
| N5a | OCR applicationに残る具体runtime/config依存を整理。既存`OcrJobDependencies`とworker/session分離を利用する | 候補SHA、manifest、checkpoint、cancel/retry、QA待ち、公開拒否が一致。fakeでの成功と実機未確認を区別 | N3/N4後。新しい統合queue・engine選定を持ち込まない |
| N5b | Capture→Libraryのメタ更新入口を先に設計し、その呼出しへ置換する。`capture_registration.py`のworkflowと`CapturePublication`の補償は保持する | `test_kindle_capture_jobs.py`/`test_kindle_capture_rollback.py`と各障害注入点で画像・メタ・job・退避世代が一致 | N5aと別の変更。N2は画像一覧のみであり、書誌更新の契約固定・保存adapterの許可依存レビューが別途必要 |
| N6 | Library/Readerの状態契約を固定し、機能公開入口を揃える。`ViewerPage.tsx`と`useReaderState.ts`を入口に、必要な表示/model/APIだけ機能内へ集約 | URL戻る/進む、書籍再開、読了、編集、見開き、一覧scroll、query無効化が一致。desktop/390px・keyboard・console受入を完了 | N2後に独立実施可。最重要の閲覧機能なので一画面・一状態単位 |
...
残る推奨順はN5a→N5b→N6→N7。N6は受入済みN2を前提に独立して進められるが、
同時に担当がいない場合は順次実施する。TM-8はMac評価CLIの別の環境待ちであり、N5aの調査・契約固定を妨げない。
N5の特定adapter・N7の互換入口撤去が未確認環境に依存する場合は、その小単位だけを保留する。
...
次の単位は**N5a: OCR applicationの具体runtime/config依存整理**とする。

1. `ocr_job_application.py`と`OcrJobDependencies`の既存テストを実行し、applicationに残るruntime/config参照を特定する。
2. 依存の組立・実行時設定参照・worker/session状態の所有者を記録する。既存の注入境界を利用し、必要な小単位だけ置き換える。
3. 候補SHA、manifest、checkpoint、cancel/retry、QA待ち、公開拒否を固定入力・隔離保存先で比較し、N3の公開処理とN4の検索回帰も維持する。
4. fakeでの制御検査と実機・実モデルの受入を分け、未確認のadapterが残ればその小単位を明記する。engine選定・queue統合・索引復旧方式変更は混在させない。
