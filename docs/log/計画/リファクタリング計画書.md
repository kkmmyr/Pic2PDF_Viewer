...
最終更新: 2026-09-08（N5aを受入し、次はN5bのメタ更新・復元境界を整理）

**現在地: N0〜N4・N5aのローカル実装・検証は完了。N5b〜N7は未着手。TM-8は環境待ち、TM-9は未着手。**
...
[N5a記録](../../archive/検証/リファクタリング_N5a_2026-09-08.md)へ移した。
...
[N5a受入記録](../../archive/検証/リファクタリング_N5a_2026-09-08.md)を参照する。
...
新計画はN系列で区別する。**下表に残すN5b以降は未着手**。N0〜N4・N5aは受入記録へ移動済み。
...
残る推奨順はN5b→N6→N7。N6は受入済みN2を前提に独立して進められるが、
同時に担当がいない場合は順次実施する。TM-8はMac評価CLIの別の環境待ちであり、N6の契約固定を妨げない。
...
次の単位は**N5b: Capture→Libraryのメタ更新・復元入口の整理**とする。

1. `CapturePublication`の同名/ASIN置換検査、既存フィールド保持、新規entryの既定値、更新前snapshot、失敗後の復元/削除を先に固定する。
2. メタ情報の読取り・ASIN更新・復元をLibrary側の狭い操作として設計する。`meta_store`へ委譲するadapterだけに依存を許可し、他のLibrary moduleへ許可を広げない。
3. `capture_registration.py`の処理順と`CapturePublication`の補償順を維持し、既存の失敗注入点で新規登録と既存置換を比較する。
4. N5aのOCR候補保存・QA待ち・公開拒否、N3の公開・N4の検索を回帰対象とし、隔離DB/画像での確認とWindows実撮影を区別する。

N5aの受入範囲は[N5a受入記録](../../archive/検証/リファクタリング_N5a_2026-09-08.md)を参照する。
