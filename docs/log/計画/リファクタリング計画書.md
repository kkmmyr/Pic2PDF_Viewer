...
最終更新: 2026-09-12（N0〜N6をmasterへ取り込み、N7a調査からN7bの検査・台帳補強を具体化）

**現在地: N0〜N6は検証・master取り込み済み。N7a調査は完了し、N7bは未着手。TM-8は環境待ち、TM-9/TM-11は未着手。**
...
新計画はN系列で区別する。**N7は進行中で、N7a調査が完了。次はN7b**。N0〜N6とN7aは記録へ移動済み。
ownerはproject owner、設計判断・統合・最終検証はメイン担当が行う。
各段階はさらに独立して検査が通る小単位へ分け、前段の受入後に次の範囲を確定する。

| 段階 | 実施内容・主な所有範囲 | 受入条件 | 依存・優先度 |
|---|---|---|---|
| N7b | 固定パス6境界の対象存在・走査保証、移動/欠落の回帰test、page FTS公開窓口の台帳登録 | 現行配置で検査成功。未登録移動・欠落を検出し、登録した新配置でも禁止importを検出。台帳と設計の同期 | 次に着手。productionソースの移動なし |
| N7c候補 | Full Build内部2ファイルを`novel_db/generation/`へ配置し、公開`full_builder.py`を維持 | 保存・失敗・型identity・公開patch契約を維持。対象の新規循環なし。検査とfile mapを更新 | N7b受入後に採否確定。機械的移動と内部変更は別commit |
...
次の単位は**N7b: 移動前の依存検査と互換台帳の補強**とする。
参照件数、固定パス検査のskip再現、公開入口と外部環境の未確認事項は
[N7a調査記録](../../archive/検証/リファクタリング_N7a_2026-09-12.md)に保存した。

1. `check_import_boundaries.py`の6つのnovel専用境界について、実リポジトリの対象存在と
   検査範囲を保証する。欠落・未登録移動で成功せず、登録した新配置で禁止importを検出するtestを追加する。
   部分fixtureの既存testと全体の存在検査を区別し、allowlistは緩めない。
2. `page_fts.py`の14公開名（loggerを含む）とproduction/CLI利用を根拠に、
   `maintenance_assets.json`と保守資産・互換層台帳へ維持する入口として登録する。
3. 検査と台帳のtest、月次監査、設計書検査を通過させる。N7bではproductionファイルを動かさず、
   package化は別commit・別受入のN7c候補とする。
4. N7c候補は`full_build_content.py`と`full_build_repository.py`を`generation/`配下の同名fileへ移す
   最小案とする。`full_builder.py`の公開名・patch先、transactionと索引更新順を維持する。
   新packageにruntime組立や大量再公開を追加しない。旧内部パスの外部契約維持が必要と分かった場合は採否を再検討する。
