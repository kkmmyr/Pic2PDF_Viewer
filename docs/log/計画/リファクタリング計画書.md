...
最終更新: 2026-09-12（N7bの検査・台帳補強を受入し、N7cの最小配置整理を採用）

**現在地: N0〜N6はmaster取り込み済み。N7aはcommit済み、N7bはローカル実装・検証完了（未commit）。N7cは採用・未着手。TM-8は環境待ち、TM-9/TM-11は未着手。**
...
新計画はN系列で区別する。**N7bはローカル受入済み。次のN7cはFull Build内部2ファイルに限定して採用**。
ownerはproject owner、設計判断・統合・最終検証はメイン担当が行う。
各段階はさらに独立して検査が通る小単位へ分け、前段の受入後に次の範囲を確定する。

| 段階 | 実施内容・主な所有範囲 | 受入条件 | 依存・優先度 |
|---|---|---|---|
| N7c | Full Build内部2ファイルを`novel_db/generation/`へ配置し、公開`full_builder.py`を維持 | 保存・失敗・型identity・公開patch契約を維持。対象の新規循環なし。検査とfile mapを更新 | 採用・未着手。N7b受入後、機械的移動を独立commitにする |
| N7後続候補 | 検索・page FTS内部配置とCLI参照を個別評価。互換撤去は外部利用条件を別途確認 | 効果を確認した範囲のみ実施。旧公開入口は維持または台帳の撤去条件を充足。関連環境受入と復旧を記録 | N7cの結果で再評価。外部環境依存の単位だけ保留 |
...
N7bをcommitして区切った後、次の単位は**N7c: Full Build内部2ファイルの配置整理**とする。
参照件数と外部環境の未確認事項は[N7a調査記録](../../archive/検証/リファクタリング_N7a_2026-09-12.md)、
検査補強と採用理由は[N7b記録](../../archive/検証/リファクタリング_N7b_2026-09-12.md)を参照する。

1. `full_build_content.py`と`full_build_repository.py`を`generation/`配下の同名fileへ移す。
   生成結果の検査とSQL保存の所在をまとめ、処理順を所有する`full_builder.py`を既存の公開入口として残す。
   新packageにruntime組立や大量再公開を追加しない。`character_names.py`、文脈生成、検索、OCR workerは移動しない。
2. `full_builder.py`と公開保存契約testのimport、contentから`character_names`への相対import、
   `NOVEL_MODULE_PATHS`の2登録とrepositoryのcontent許可先を更新する。依存を追加で許可する変更は混ぜない。
3. `test_novel_boundary_targets.py`・Full Build境界testのfixtureを新配置へ合わせ、
   全6対象の存在、削除/未登録移動の拒否、移動先の許可/禁止importを再確認する。
   公開入口・型identity・patch先・保存成功/失敗・commit後索引更新と通知順を既存testで比較する。
4. パイプライン設計、品質ガードレール、自動file mapを更新する。
   対象の静的依存と隔離環境での公開importを比較し、新規循環がないことを確認して全体検査を通す。
5. 機械的移動を独立したcommitにする。旧内部パスの外部利用・保存済みPython型依存が判明した場合は
   wrapperを自動追加せず停止する。API・DB migration・保存方式変更が必要ならN7cの採否から再検討する。

ownerはproject owner。N7cは小範囲での責務の見通しを改善するための採用であり、開発速度の改善を
実測したという意味ではない。検索・page FTS・CLIの後続配置はN7c受入後に改めて採否を判断する。
...
TM-11で別途扱い、N7へ混在させない。実モデル・実撮影の受入を追加完了として扱わず、詳細は
