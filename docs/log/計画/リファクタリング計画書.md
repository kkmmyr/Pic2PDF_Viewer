...
最終更新: 2026-09-08（N0〜N2を受入し、N3以降の保存境界とTM-9を整理）

**現在地: N0〜N2のローカル実装・検証は完了。N3〜N7は未着手。TM-8は環境待ち、TM-9は未着手。**
完了範囲と検証は[N0〜N2記録](../../archive/検証/リファクタリング_N0-N2_2026-09-08.md)へ移した。
N0の契約固定は今回の起動・画像一覧を対象とし、後続でも各処理の保存・失敗境界を着手前に固定する。
...
この表は計画策定時のsnapshotである。N0〜N2による変更後の所有境界・比較値は
[受入記録](../../archive/検証/リファクタリング_N0-N2_2026-09-08.md)を参照する。
...
新計画はN系列で区別する。**下表に残すN3以降は未着手**。N0〜N2は受入記録へ移動済み。
ownerはproject owner、設計判断・統合・最終検証はメイン担当が行う。
各段階はさらに独立して検査が通る小単位へ分け、前段の受入後に次の範囲を確定する。

| 段階 | 実施内容・主な所有範囲 | 受入条件 | 依存・優先度 |
|---|---|---|---|
| N3 | `full_builder.py`の生成・検査・保存を分ける。まず既存commit/rollbackとcommit後索引更新の境界を固定し、生成結果の型とSQL保存入口を作る | skip/redo、生成失敗、人物消失の拒否、SQL失敗、索引失敗の前後状態・job通知が一致。`test_novel_db_full_builder.py`とcontext回帰を維持 | N0〜N2の方法を適用。汎用repository化やtransaction位置変更は混在させない |
| N4 | `search.py`の検索・統合と結果整形を分ける。FTS/vector接続を差し替え、API向けsnippet/画像URLの所有先を確定する | 固定入力でscope、順位・同点処理、重複除去、fallback、出典、snippet、URLを比較。既存page FTSの状態遷移は変えない | N3後。N3と検索索引の所有を合わせる |
...
残る推奨順はN3→N4→N5a→N5b→N6→N7。N6は受入済みN2を前提に独立して進められるが、
同時に担当がいない場合は順次実施する。TM-8はMac評価CLIの別の環境待ちであり、N3/N4の開始を妨げない。
...
次の単位は**N3: Full Buildの生成結果・検査・保存境界の固定**とする。

1. `test_novel_db_full_builder.py`とcontext回帰を実行し、skip/redo・失敗時のSQLiteと索引の状態を確認する。
2. DB読取り、生成呼出し、人物検査、SQL置換、commit、索引更新の各所有者と境界を設計へ記録する。
3. 未網羅の失敗点をfixtureで固定した後、生成結果の型と小さな保存関数を抽出する。
4. 同等性を確認してから呼出し元を置換する。commit位置変更、索引の新しい復旧方式、全DBの共通化は別の判断とする。
