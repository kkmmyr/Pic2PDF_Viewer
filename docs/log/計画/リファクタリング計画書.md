...
最終更新: 2026-09-08（N3を受入し、次のN4と索引の保存境界を整理）

**現在地: N0〜N3のローカル実装・検証は完了。N4〜N7は未着手。TM-8は環境待ち、TM-9は未着手。**
完了範囲と検証は[N0〜N2記録](../../archive/検証/リファクタリング_N0-N2_2026-09-08.md)と
[N3記録](../../archive/検証/リファクタリング_N3_2026-09-08.md)へ移した。
...
[N3受入記録](../../archive/検証/リファクタリング_N3_2026-09-08.md)を参照する。
...
新計画はN系列で区別する。**下表に残すN4以降は未着手**。N0〜N3は受入記録へ移動済み。
...
残る推奨順はN4→N5b→N6→N7。N6は受入済みN2を前提に独立して進められるが、
同時に担当がいない場合は順次実施する。TM-8はMac評価CLIの別の環境待ちであり、N4の開始を妨げない。
...
次の単位は**N4: 検索・統合とAPI向け結果整形の分離**とする。

1. 検索scope、順位・同点、重複、fallback、出典、snippet/画像URLの既存契約を固定する。
2. FTS/vector接続・結果統合・API向け整形の所有者を定義し、小単位で分離する。
3. N3の公開commit後索引更新とpage全文索引の状態遷移を維持する。
4. SQLite公開済みでも索引が古い・欠落し得る状態を前提に検証する。通常再実行による自動修復の追加は別の変更として判断する。
