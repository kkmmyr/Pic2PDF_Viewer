...
最終更新: 2026-09-12（TM-11をmasterへ取り込み、Linux配備・通常停止・隔離実データ復旧を受入）

**現在地: N0〜N7c・TM-9・TM-11はmaster取り込みとLinux配備済み（TM-11はPR #25、b81b3a7）。通常停止と隔離実データでの復旧・再登録を確認済み。Windows新規実撮影は対象書籍の指定待ち。追加の物理移動は見送り、互換撤去は外部確認待ち。TM-8は環境待ち。**
...
Windows新規実撮影は対象書籍名またはASINの指定待ち。指定後にagentの起動条件と画像置換対象を照合して実施する。
...
次は対象書籍を指定したWindows新規実撮影。TM-8・互換撤去の外部確認は環境・利用状況の確認後に進める。
