...
export function SeriesEditDialog() {
    const { seriesEdit, seriesEditFilteredSeries, getSeries, currentPath } =
        useLibraryPanelContext();

    const open = seriesEdit.target !== null;
    const targetName = seriesEdit.target ?? '';
    const current = seriesEdit.target ? getSeries(currentPath, seriesEdit.target) : null;
    const allSeries = seriesEditFilteredSeries;
    const onClose = seriesEdit.close;
    const onAssign = seriesEdit.assign;
    const onUnassign = seriesEdit.unassign;
