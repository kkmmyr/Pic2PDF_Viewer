...
describe('useSortedPdfs', () => {
    ...
    describe('favorites_first', () => {
        it('お気に入りを先頭に、それ以外は名前昇順', () => {
            ...
            expect(sortedNames(pdfs, 'favorites_first', favorites))
    ...
    describe('view_desc', () => {
        it('閲覧回数の降順、同数は名前昇順', () => {
            ...
            const counts = new Map([['a', 1], ['b', 5], ['c', 5], ['d', 0]]);
            const getViewCount = (name: string) => counts.get(name) ?? 0;
            expect(sortedNames(pdfs, 'view_desc', new Set(), getViewCount))
                .toEqual(['b', 'c', 'a', 'd']);
        });

        it('getViewCount 未指定なら全て 0 として名前昇順', () => {
            const pdfs = [pdf('Charlie'), pdf('Alpha'), pdf('Bravo')];
            expect(sortedNames(pdfs, 'view_desc'))
    ...
    describe('recent_view', () => {
        it('最終閲覧時刻の降順、未閲覧は末尾、同時刻は名前昇順', () => {
            ...
            expect(sortedNames(pdfs, 'recent_view', new Set(), undefined, getLastViewedAt))
                .toEqual(['b', 'd', 'a', 'c']);
        });
