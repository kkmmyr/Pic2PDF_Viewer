...
describe('useLibraryPins', () => {
    describe('初期状態', () => {
        ...
        it('localStorage に既存データがあれば復元する', () => {
            ...
            localStorage.setItem('pins_author_generated', JSON.stringify({ 'Author A': 'bookA.pdf' }));
            const { result } = renderHook(() => useLibraryPins('generated'));
    ...
    describe('toggleSeriesPin', () => {
        it('未ピン状態でトグルするとピンが追加される', () => {
            const { result } = renderHook(() => useLibraryPins('generated'));
            act(() => { result.current.toggleSeriesPin('sid-1', 'vol1.pdf'); });
        ...
        it('別の書籍をトグルすると代表が切り替わる（1グループ1冊のみ）', () => {
            const { result } = renderHook(() => useLibraryPins('generated'));
            act(() => { result.current.toggleSeriesPin('sid-1', 'vol1.pdf'); });
            act(() => { result.current.toggleSeriesPin('sid-1', 'vol3.pdf'); });
            expect(result.current.seriesPins['sid-1']).toBe('vol3.pdf');
        });

        it('localStorage に保存される', () => {
            const { result } = renderHook(() => useLibraryPins('generated'));
            act(() => { result.current.toggleSeriesPin('sid-1', 'vol2.pdf'); });
    ...
    describe('toggleAuthorPin', () => {
        it('未ピン状態でトグルするとピンが追加される', () => {
            const { result } = renderHook(() => useLibraryPins('generated'));
            act(() => { result.current.toggleAuthorPin('Author A\nAuthor B', 'bookA.pdf'); });
            expect(result.current.authorPins['Author A\nAuthor B']).toBe('bookA.pdf');
        });

        it('同じ書籍を再トグルするとピンが解除される', () => {
            const { result } = renderHook(() => useLibraryPins('generated'));
            act(() => { result.current.toggleAuthorPin('Author A', 'bookA.pdf'); });
            act(() => { result.current.toggleAuthorPin('Author A', 'bookA.pdf'); });
        ...
        it('別の書籍をトグルすると代表が切り替わる', () => {
            const { result } = renderHook(() => useLibraryPins('generated'));
            act(() => { result.current.toggleAuthorPin('Author A', 'bookA.pdf'); });
            act(() => { result.current.toggleAuthorPin('Author A', 'bookB.pdf'); });
    ...
    describe('ソース別の独立管理', () => {
        it('source が異なれば別のキーで管理される', () => {
            ...
            act(() => { gen.current.toggleSeriesPin('sid-1', 'vol1.pdf'); });
