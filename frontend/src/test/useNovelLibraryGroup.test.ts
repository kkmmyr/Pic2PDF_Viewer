...
describe('useNovelLibraryGroup', () => {
    describe('flat モード', () => {
        it('全書籍が ungrouped に入り groups は空', () => {
            const { result } = renderHook(() =>
                useNovelLibraryGroup([A, B, C, D], 'flat'),
            );
    ...
    describe('author モード', () => {
        it('第 1 作者でグループ化される', () => {
            const { result } = renderHook(() =>
                useNovelLibraryGroup([A, B, C], 'author'),
            );
        ...
        it('作者未設定の書籍は ungrouped に入る', () => {
            const { result } = renderHook(() =>
                useNovelLibraryGroup([A, D], 'author'),
            );
        ...
        it('グループはラベル昇順でソートされる', () => {
            const { result } = renderHook(() =>
                useNovelLibraryGroup([C, A], 'author'),
            );
    ...
    describe('series モード', () => {
        it('series_id でグループ化される', () => {
            const { result } = renderHook(() =>
                useNovelLibraryGroup([A, B, C], 'series'),
            );
        ...
        it('シリーズ内は volume 昇順にソートされる', () => {
            ...
            const { result } = renderHook(() =>
                useNovelLibraryGroup([bVol3, bVol1], 'series'),
            );
        ...
        it('volume が null の書籍はシリーズ末尾に入る', () => {
            ...
            const { result } = renderHook(() =>
                useNovelLibraryGroup([bNull, bVol1], 'series'),
            );
            const s1 = result.current.groups[0];
        ...
        it('シリーズ未設定の書籍は ungrouped に入る', () => {
            const { result } = renderHook(() =>
                useNovelLibraryGroup([A, D, E], 'series'),
            );
            expect(result.current.ungrouped).toContainEqual(D);
