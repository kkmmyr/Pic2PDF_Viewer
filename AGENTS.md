...
同人誌・漫画・小説を対象としたマルチソース閲覧Webアプリ。画像・ZIPの取り込み、画像/PDF閲覧、Kindle自動撮影連携、小説OCR・QA・本文公開、EmbeddingとQwenによるRAG検索・QA・人物辞典・要約を備える。
**sourceの3値: `doujin` / `comic` / `novel`**。現行機能と設計の入口は[docs/index.md](docs/index.md)。
...
- リポジトリ共通の起動・配備用`.bat` / `.sh`は **`scripts/` 配下**。`backend/start_server.bat`と`kindle-pdf/`のモジュール専用スクリプトは各モジュール内にある
...
設計書の生成手順は[設計書運用ルール](docs/design/環境構築/設計書運用ルール.md#docs-build)、配信構成は共通設計を参照する。サイドバーは目的別に分類し、物理配置の3バケットと区別する。過去の全記録は[アーカイブ索引](docs/archive/index.md)から辿る。
