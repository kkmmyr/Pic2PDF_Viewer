...
`OllamaBackend` / `LlamaServerBackend` を instantiate するため、こちらは使わない。
...
def backend_from_env() -> Backend:
    ...
    | 変数 | 既定 | 用途 |
    |---|---|---|
    | `QWEN_BACKEND` | `llama_server` | `llama_server` / `ollama` を選択 |
